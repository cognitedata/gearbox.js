...
### Ydomain Configurations

```jsx
const yDomainScaleOptions = { 123: { min: -200, max: 200 } };
```

<Preview>
  <Story name="Ydomain Configurations" >
    <TimeseriesChart
      timeseriesIds={[123, 456]}
      yDomainScaleOptions = {yDomainScaleOptions}
    />
  </Story>
</Preview>

### Custom Series Ydomain Configurations

```jsx
import { AxisDisplayMode } from '@cognite/griff-react';
import { DataLoader } from '@cognite/gearbox';

const customYdomainSeries = [
  {
    id: 123,
    color: 'green',
    yAxisDisplayMode: AxisDisplayMode.ALL,
...
<Preview>
  <Story name="Custom Series Ydomain Configurations" >
    <TimeseriesChart
      series={customYdomainSeries} 
    />
  </Story>
</Preview>
