...
const FileInputComponent = ({ render }: any) => {
...
const renderer = (image: any) => <AssetScanner onError={onError} image={image} />;
...
const getAssetHandlerCustom = async (strings: string[]): Promise<Asset[]> => {/* ... */}
```
