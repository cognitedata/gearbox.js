...
```typescript
const handleYDomainchange = (data: YDomainsChange) =>
...
```
<Preview>
  <Story name="Y-Domain Changed" decorators={[zoomableClientDecorator]}>
    <TimeseriesChart
      timeseriesIds={[123]}
      startTime={Date.now() - 30 * 24 * 60 * 60 * 1000}
      endTime={Date.now()}
      zoomable={true}
      onUpdateYDomain={handleYDomainchange}
      contextChart={true}
    />
  </Story>
</Preview>
